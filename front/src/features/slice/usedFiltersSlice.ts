...
const usedFiltersSlice = createSlice({
  ...
  reducers: {
    ...
    toggleFilter: (state, action: PayloadAction<IFilterPayload>) => {
      const { filter, filterValue, catId, subcatId, subcatList } =
        action.payload;
      switch (filter) {
        case 'category':
          if (catId) {
            if (state.dataFilter.categories[catId]) {
              //клик по категории которая активна (либо сама либо подкатегории)
              delete state.filters[filterValue];
        ...
        case 'subcategory':
          if (catId && subcatId) {
            ...
            if (!state.dataFilter.categories[catId].includes(subcatId)) {
              state.dataFilter.categories[catId].push(subcatId);
              state.filters[filterValue] = ['subcategory', catId, subcatId];
            } else {
              state.dataFilter.categories[catId] = state.dataFilter.categories[
                catId
              ].filter((value) => value !== subcatId);
              delete state.filters[filterValue];
            }
          }
          break;
        ...
        case 'city':
          if (catId) {
            if (state.dataFilter.cities.includes(catId)) {
              //если город есть то удаляем
              state.dataFilter.cities = state.dataFilter.cities.filter(
                (value) => value !== catId,
              );
              delete state.filters[filterValue];
            } else {
...
export const {
  ...
  toggleFilter,
  clearFilters,
} = usedFiltersSlice.actions;

export default usedFiltersSlice.reducer;

export const selectUsedFilters = (state: RootState) =>
  state.usedFilters.filters;
export const selectDataFilter = (state: RootState) =>
  state.usedFilters.dataFilter;
export const selectIsFilterActive = (state: RootState) =>
  state.usedFilters.isFilterActive;
